...
def promote(update: Update, context: CallbackContext) -> str:
    ...
    if not user_id:
        message.reply_text(
            "You don't seem to be referring to a user or the ID specified is not correct.."
        )
        return log_message
    ...
    if user_member.status == 'administrator' or user_member.status == 'creator':
        message.reply_text(
            "How am I can promote someone that's already an admin?")
        return log_message

    if user_id == bot.id:
        message.reply_text(
            "I can't promote myself! Get an admin to do this for me.")
        return log_message
    ...
    try:
        ...
        if err.message == "User_not_mutual_contact":
            ...
            message.reply_text("An unknown error occured while promoting.")
            return log_message
...
def demote(update: Update, context: CallbackContext) -> str:
    ...
    if user_member.status == 'creator':
        message.reply_text(
            "This person has created this chat, how would I demote them?")
        return log_message

    if not user_member.status == 'administrator':
        message.reply_text("Can't demote whom i didn't promoted!")
        return log_message
...
def set_title(update: Update, context: CallbackContext):
    ...
    if user_member.status == 'creator':
        message.reply_text(
            "This person has Created the chat, how can i set custom title for him?")
        return
    ...
    bot.sendMessage(
        chat.id,
        f"Sucessfully set the  title for! <code>{user_member.user.first_name or user_id}</code> "
