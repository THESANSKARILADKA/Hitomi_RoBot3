...
async def download_song(v_url):
    ...
    type = "audio"
    await rkp.edit("`Preparing to download ...Join @Hitomi_Support For Reporting Bugs`")
    if type == "audio":
        opts = {
            'format':
            'bestaudio',
    ...
    c_time = time.time()
    if song:
        await rkp.edit(f"`Sending the song ...Plz Wai8`")
...
__help__ = """
 - /gana <songname artist(optional)>: uploads the song in it's best quality available
 - /vid <songname artist(optional)>: uploads the video song in it's best quality available
